    ...
    public static class AppBarFunctions
    {
        ...
        private class RegisterInfo
        {
                                    ...
                                    IntPtr lParam, ref bool handled)
            {
                if (msg == CallbackId)
                {
                    if (wParam.ToInt32() == (int)NativeMethods.ABNotify.ABN_POSCHANGED)
                    {
        ...
        public static void SetAppBar(Window appbarWindow, ABEdge edge, FrameworkElement childElement = null)
        {
            ...
            var wiHelper = new WindowInteropHelper(appbarWindow);
            wiHelper.EnsureHandle();

            var abd = new NativeMethods.APPBARDATA();
            abd.cbSize = Marshal.SizeOf(abd);
            abd.hWnd = wiHelper.Handle;
            ...
            if (edge == ABEdge.None)
            {
                ...
                renderPolicy = (int)NativeMethods.DWMNCRenderingPolicy.UseWindowStyle;

                NativeMethods.DwmSetWindowAttribute(abd.hWnd, (int)NativeMethods.DWMWINDOWATTRIBUTE.DWMA_EXCLUDED_FROM_PEEK, ref renderPolicy, sizeof(int));
                NativeMethods.DwmSetWindowAttribute(abd.hWnd, (int)NativeMethods.DWMWINDOWATTRIBUTE.DWMA_DISALLOW_PEEK, ref renderPolicy, sizeof(int));
            ...
            if (!info.IsRegistered)
            {
                ...
                info.CallbackId = NativeMethods.RegisterWindowMessage("AppBarMessage");
                abd.uCallbackMessage = info.CallbackId;

                var ret = NativeMethods.SHAppBarMessage((int)NativeMethods.ABMsg.ABM_NEW, ref abd);
            ...
            renderPolicy = (int)NativeMethods.DWMNCRenderingPolicy.Enabled;
        ...
        {
            var barData = new NativeMethods.APPBARDATA();
            ...
            NativeMethods.SHAppBarMessage((int)NativeMethods.ABMsg.ABM_QUERYPOS, ref barData);
            NativeMethods.SHAppBarMessage((int)NativeMethods.ABMsg.ABM_SETPOS, ref barData);
