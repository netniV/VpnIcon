    ...
    public sealed class RasAutoDialAddressCollection : RasCollection<RasAutoDialAddress>
    {
        ...
        internal void Load()
        {
            try
            {
                ...
                Collection<string> addresses = RasHelper.Default.GetAutoDialAddresses();
                if (addresses != null && addresses.Count > 0)
                {
                    for (int index = 0; index < addresses.Count; index++)
                    {
                        RasAutoDialAddress item = RasHelper.Default.GetAutoDialAddress(addresses[index]);
                        if (item != null)
                        {
                            this.Add(item);
                        }
        ...
        protected override void InsertItem(int index, RasAutoDialAddress item)
        {
            ...
            {
                if (RasHelper.Default.SetAutoDialAddress(item.Address, item.Entries))
                {
                    ...
                    item = RasHelper.Default.GetAutoDialAddress(item.Address);
        ...
        protected override void RemoveItem(int index)
        {
            if (!this.IsInitializing)
            {
                ...
                if (item != null)
                {
                    if (RasHelper.Default.SetAutoDialAddress(item.Address, null))
                    {
