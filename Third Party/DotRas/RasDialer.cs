    ...
    public sealed partial class RasDialer : RasComponent
    {
                ...
                {                    
                    ...
                    try
                    {
                        ...
                        if (!string.IsNullOrEmpty(this.EntryName))
                        {
                            ...
                            {
                                data = RasHelper.Default.GetEapUserData(IntPtr.Zero, this.PhoneBookPath, this.EntryName);
                            }
                        ...
                        this.handle = RasHelper.Default.Dial(this.PhoneBookPath, parameters, extensions, callback, this.GetRasEapOptions());
        ...
        private NativeMethods.RASDIALPARAMS BuildDialParams()
        {
            ...
            if (this.Credentials == null && this.AllowUseStoredCredentials)
            {
                ...
                NetworkCredential storedCredentials = RasHelper.Default.GetCredentials(this.PhoneBookPath, this.EntryName, NativeMethods.RASCM.UserName | NativeMethods.RASCM.Password | NativeMethods.RASCM.Domain);
        ...
        private void Abort()
        {
            lock (this.syncRoot)
            {
                if (this.handle != null && !this.handle.IsInvalid)
                {
                    ...
                    RasHelper.Default.HangUp(this.handle, this.HangUpPollingInterval, false);
                }
            }
        }
